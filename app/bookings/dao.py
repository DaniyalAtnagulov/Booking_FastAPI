...
Представляет собой Паттерн для абстракции, содержащий в себе всё, что относится к способу хранения данных. 
Он предназначен для разделения бизнес-логики от деталей реализации слоя доступа к данным.
...
По поводу именования, точно ли service.py будет эквивалентно dao.py? Посмотрела гитхаб с best practices по FastAPI,  
там указано, что в service.py  принято хранить бизнес логику. Возможно что-то не так поняла)

 Да, действительно, в service.py/command.py обычно хранится логика приложения, а внутри dao.py/repository.py обращение к базе данных. 
 
 
Имеет смысл пройтись pyright 
"""
